...
public class SwissRaptorStopFinder implements RaptorStopFinder {
	...
	@Inject
	public SwissRaptorStopFinder(Config config, RaptorIntermodalAccessEgress intermodalAE,
									Map<String, Provider<RoutingModule>> routingModuleProviders, SimulationParameter simulationParams) {
		...
		if (srrConfig.isUseIntermodalAccessEgress()) {
			for (SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet params : srrConfig
	...
	private List<InitialStop> findAccessStops(Facility fromFacility, Facility toFacility, Person person, double departureTime,
		...
		if (srrCfg.isUseIntermodalAccessEgress()) {
			...
			List<InitialStop> initialStops = stops.stream().map(stop -> {
				...
				return new InitialStop(stop, disutility, travelTime, beelineDistance * distanceFactor,
						TransportMode.walk);
	...
	private List<InitialStop> findEgressStops(Facility fromFacility, Facility toFacility, Person person, double departureTime,
											  RaptorParameters parameters, SwissRailRaptorData data) {
		SwissRailRaptorConfigGroup srrCfg = parameters.getConfig();
		if (srrCfg.isUseIntermodalAccessEgress()) {
	...
	private List<InitialStop> findIntermodalStops(Facility fromFacility, Facility toFacility, Person person, double departureTime,
		...
		switch (srrCfg.getIntermodalAccessEgressModeSelection()) {
			case CalcLeastCostModePerStop:
				for (SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet parameterSet : srrCfg
						.getIntermodalAccessEgressParameterSets()) {
			...
			case RandomSelectOneModePerRoutingRequestAndDirection:
				...
				do {
					...
					counter++;
					// try again if no initial stop was found for the parameterset. Avoid infinite
					// loop by limiting number of tries.
	...
	private void addInitialStopsForParamSet(Facility fromFacility, Facility toFacility, Person person, double departureTime, Direction direction,
											RaptorParameters parameters, SwissRailRaptorData data, double x, double y, List<InitialStop> initialStops,
											SwissRailRaptorConfigGroup.IntermodalAccessEgressParameterSet paramset) {
		...
		if (personMatches) {
			...
			if (stopFilterAttribute != null) {
				if (data.stopFilterAttribute2Value2StopsQT.containsKey(stopFilterAttribute)) {
					if (data.stopFilterAttribute2Value2StopsQT.get(stopFilterAttribute).containsKey(stopFilterValue))
						filteredStopsQT = data.stopFilterAttribute2Value2StopsQT.get(stopFilterAttribute)
								.get(stopFilterValue);
					else {
			...
			for (TransitStopFacility stop : stopFacilities) {
				...
				if (linkIdAttribute != null) {
					...
					if (attr != null) {
						stopFacility = new ChangedLinkFacility(stop,
								Id.create(attr.toString(), Link.class));
					}
				}

				//@kaghog
				// Configure Minimum Access Distance for non walk modes
				// check for if the stop facility is within shortest stop distance from origin for non walk
				double stopDistance = CoordUtils.calcEuclideanDistance(facility.getCoord(), stop.getCoord());
				double minAccessDistance = simulationParams.getMinAccessDist();
				if (!(mode.equals(TransportMode.walk)) && stopDistance < minAccessDistance) {
					continue;
				}
				...
				if (direction == Direction.ACCESS) {
					...
					// We don't know the departure time for the egress trip, so just use the
					// original departureTime,
					// although it is wrong and might result in a wrong traveltime and thus wrong
					// route.
					RoutingModule module = this.routingModules.get(mode);
					...
					if (routeParts == null) {
						// the router for the access/egress mode could not find a route, skip that
						// access/egress mode
						continue;
					}
				...
				if (stopFacility != stop) {
					if (direction == Direction.ACCESS) {
						...
						Route transferRoute = RouteUtils.createGenericRouteImpl(stopFacility.getLinkId(),
								stop.getLinkId());
						...
						Leg transferLeg = PopulationUtils.createLeg(TransportMode.walk);
						Route transferRoute = RouteUtils.createGenericRouteImpl(stop.getLinkId(),
				...
				RaptorIntermodalAccessEgress.RIntermodalAccessEgress accessEgress = ((SwissRaptorIntermodalAccessEgress) this.intermodalAE)
						.calcIntermodalAccessEgress(routeParts, parameters, person, direction, departureTime);
				InitialStop iStop = new InitialStop(stop, accessEgress.disutility, accessEgress.travelTime,
						accessEgress.routeParts);
